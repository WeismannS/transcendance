...
export function initializeNotificationWs() {
  ...
  const ws = new WebSocket(`ws://localhost:3005/ws/notifications/live?token=${encodeURIComponent(token)}`);
  ...
  ws.onmessage = (event) => {
    try {
      ...
      if (isNotificationType(data, "FRIEND_REQUEST_RECEIVED")) {
        addNotification({ title: data.title, avatar: data.user.avatar, type: "info", message: data.content });
        stateManager.emit("FRIEND_REQUEST_RECEIVED", { user: data.user, id: data.requestId });
        ...
        addNotification({ title: data.title, type: "info", message: data.content, onView: () => redirect("/tournaments") });
  ...
  ws.onclose = () => {
    ...
    setTimeout(() => { initializeNotificationWs(); }, 5000);
  };
