...
export async function createTournament(tournamentData: { name: string; status: string; startTime: string; username: string; }) {
  try {
...
export async function stopTournament(tournamentId: string) {
  try {
    const response = await fetch(API_URL + `/api/tournament/${tournamentId}/stop`, {
      method: "POST",
      credentials: "include",
    });
...
export async function getTournamentMatches(tournamentId: string) {
  try {
    const response = await fetch(API_URL + `/api/tournament/${tournamentId}/matches`, {
      method: "GET",
      credentials: "include",
    });
