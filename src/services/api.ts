  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        throw new Error(
          errorData.error ||
            "One or both players are already in an active game. Please wait for the current game to end."
        );
      }
...
// Clear stuck game sessions for the current user
export async function clearMyGameSessions() {
  try {
    // Get current user ID
    const userData = stateManager.getState("userProfile") as UserProfileState;
    if (!userData?.id) {
      throw new Error("User not authenticated");
    }

    const response = await fetch(
      API_URL + "/api/game/sessions/" + userData.id,
      {
        method: "DELETE",
        credentials: "include",
      }
...
export const gameConnect = (userId, gameId, callbacks) => {
  return new Promise((resolve, reject) => {
    // Construct WebSocket URL - adjust the host/port as needed
    const wsUrl = `ws://localhost:3006/ws?playerId=${userId}&gameId=${gameId}`;
    ...
    socket.onopen = (event) => {
      ...
      // IMPORTANT: Resolve with the socket instance
      resolve(socket);
    };
